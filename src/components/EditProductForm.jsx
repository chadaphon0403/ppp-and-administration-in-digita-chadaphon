...
export default function EditProductForm({
  ...
  const [newProject, setNewProject] = useState(image);
  ...
  const [newEvaluation, setNewEvalution] = useState(evaluation);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/products/${id}`, {
        ...
        body: JSON.stringify({ newName, newProject, newImplementation, newEmail, newMobile, newBudget, newYear, newEvaluation, newStrength, newWeak, newDevelopment, newSuggestion }),
      });
